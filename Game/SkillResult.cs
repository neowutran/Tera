...
namespace Tera.Game
{
    public class SkillResult
    {
        public SkillResult(EachSkillResultServerMessage message, EntityTracker entityRegistry,
            ...
            AbnormalityTracker abnormalityTracker = null)
        {
            Time = message.Time;
            ...
            if (Source is PlaceHolderEntity && (Target as NpcEntity)?.Info.Boss == true)
                Source = playerTracker.GetUnknownPlayer() ??
                         Source; //track unknown damage dealt to bosses like in raid-30
            ...
            var sourceUser =
                userNpc["root_source"] as UserEntity; // Attribute damage dealt by owned entities to the owner
            ...
            if (sourceUser != null)
            {
                ...
                if (Skill == null && npc != null)
                    Skill = petSkillDatabase?.GetOrNull(npc.Info, SkillId) ?? new UserSkill(message.SkillId,
                                sourceUser.RaceGenderClass, npc.Info.Name, null,
                                "",
                ...
                if (SkillId == 51001 && (abnormalityTracker?.AbnormalityExist(Source.Id, 10152050) ?? false))
                {
//gunner burstfire buff detection
                    SkillId = SkillId + abnormalityTracker.AbnormalityStorage.Get(SourcePlayer).Times
                                  .Last(x => x.Key.Id == 10152050).Value.LastStack();
            ...
            if (SourcePlayer != null && npc != null)
                ...
                {
                    if ((HitDirection = userNpc["root_source"].Heading.HitDirection(Target.Heading)) != Source.Position
                            .GetHeading(Target.Position)
                            .HitDirection(Target.Heading)) HitDirection = HitDirection.Front;
                }
            else if ((HitDirection = Source.Heading.HitDirection(Target.Heading)) !=
                     Source.Position.GetHeading(Target.Position).HitDirection(Target.Heading))
                HitDirection = HitDirection.Front;
            if (SourcePlayer?.Class == PlayerClass.Archer &&
                (abnormalityTracker?.AbnormalityExist(sourceUser.Id, 601600) ?? false))
