...
namespace Tera.Game
{
    ...
    {
        private readonly Dictionary<Tuple<ushort, uint>, List<UserSkill>> _petSkilldata =
            new Dictionary<Tuple<ushort, uint>, List<UserSkill>>();
        ...
        public PetSkillDatabase(string folder, string language, NpcDatabase npcDatabase)
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                var skill = new UserSkill(skillId, new RaceGenderClass(Race.Common, Gender.Common, PlayerClass.Common),
                    skillName, null, string.IsNullOrWhiteSpace(petName) ? npcinfo.Name : petName, icon, npcinfo);
