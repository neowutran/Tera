...
namespace Tera.Game
{
    public class AbnormalityStorage
    {
        ...
        public AbnormalityStorage(Dictionary<NpcEntity, Dictionary<HotDot, AbnormalityDuration>> npcTimes, Dictionary<Player, Dictionary<HotDot, AbnormalityDuration>> playerTimes, Dictionary<Player, Death> playerDeath, Dictionary<Player, Dictionary<NpcEntity, Death>> playerAggro)
        {
        ...
        {
            if (player == null) new PlayerAbnormals();
            return new PlayerAbnormals(
                !PlayerAbnormalityTime.ContainsKey(player) ? new Dictionary<HotDot, AbnormalityDuration>() : PlayerAbnormalityTime[player],
        ...
        public AbnormalityStorage Clone()
        {
            ...
            var PlayerTimes = PlayerAbnormalityTime.ToDictionary(y => y.Key, y => y.Value.ToDictionary(x => x.Key, x => (AbnormalityDuration)x.Value.Clone()));
            var playerDeath = PlayerDeath.ToDictionary(x => x.Key, x => x.Value.Clone());
            var playerAggro = PlayerAggro.ToDictionary(y => y.Key, y => y.Value.ToDictionary(x => x.Key, x => x.Value.Clone()));
            return new AbnormalityStorage(npcTimes, PlayerTimes, playerDeath, playerAggro);
        }
